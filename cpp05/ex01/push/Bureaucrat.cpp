...
void Bureaucrat::signForm(const Form &src)
{
    ...
    {
        std::cout << _name << " couldn’t sign " << src.getName() << " because ";
        if (this->_grade > src.getRequiredGrade())
            std::cout << "is lower then the required Grade. " << std::endl;
        if (this->_grade > src.getSignGrade())
            std::cout << "is higher then the required Grade. " << std::endl;
    }
}